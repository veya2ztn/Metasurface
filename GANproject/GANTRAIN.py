...
save_checkpoint   = os.path.join(SAVEROOT,'checkpoints','GAN_PATTERN','WGAN_normal',TRIAL_NOW)
...
# modelGD_state_dict=torch.load("checkpoints/GAN_PATTERN/WGAN/27_19_34-seed-35749/weights/demo_valid_at3500")
# model.G.load_state_dict(modelGD_state_dict['D_state_dict'])
# model.D.load_state_dict(modelGD_state_dict['G_state_dict'])
...
hparam_dict   = {'d_lr': d_lr,'g_lr': g_lr,'disc_iter':disc_iter}
...
for epoch in mb:
    ...
    if generator_iter%500==0:
        ...
        z = torch.Tensor(16, 100, 1, 1).normal_(0,1).to(device)
        random_sample=np.random.choice(range(len(valid_curves)),16,replace=False).tolist()
        ces=valid_curves[random_sample].to(device)
        z[:,:curve_dim,0,0]=ces[:,0,:]
        z = Variable(z)
        model.G.eval()
        with torch.no_grad():fake_images = model.G(z)
        fake_image_figure=plot16x16(fake_images.reshape(16,16,16).cpu().detach().numpy())
    ...
    for d_iter in range(disc_iter):
        ...
        curves,images =infinity_prefetcher.next()
        ...
        z = torch.Tensor(images.size(0), 100, 1, 1).normal_(0,1).to(device)
        ...
        mb.lwrite('Step [{}/{}], d_loss: {:.4f}, g_loss: {:.4f}'.format(epoch, epoches, d_loss, g_loss),end='\r')
    train_phase(model,'G')
    ...
    logsys.recorder.add_scalars('loss',{'d_loss': d_loss,'g_loss': g_loss},generator_iter)
