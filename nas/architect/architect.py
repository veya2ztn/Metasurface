...
class Architect(ABC):
    ...
    def genotype(self, weights=None,**kargs):
        if weights is None:
            weights = self.get_weights()

        return self.model.genotype(weights,**kargs)


    def weight_distribution(self, weights=None):
        def node_map(num):
            if num == 0: return "a"
            if num == 1: return "b"
            return num-2
        ...
        normal_weight=weights["normal"]
        reduce_weight=weights["reduce"]
        normal_weight=normal_weight/(normal_weight.sum(1,keepdims=True)+1e-10)
        reduce_weight=reduce_weight/(reduce_weight.sum(1,keepdims=True)+1e-10)
        graph_fig_normal, graph_axes = plt.subplots(nrows=14, ncols=1, figsize=(4*8*1.5,1*14*1.5))
        graph_axes=graph_axes.flatten()
        edge_map  =[(node_map(i),j) for j in range(4) for i in range(j+2)]
        for i,((in_node,current_node),ax, data) in enumerate(zip(edge_map,graph_axes,normal_weight)):
            ...
            ax.set_yticks([])
            ax.set_ylabel(f"{in_node}->{current_node}",rotation=0,#fontdict={'fontsize':30},
            labelpad=50)
            if i == 13:
                ax.set_xticks(range(len(data)))
                ax.set_xticklabels(self.op_names, fontsize=28)
        graph_fig_reduce, graph_axes = plt.subplots(nrows=14, ncols=1, figsize=(4*8*1.5,1*14*1.5))
        graph_axes=graph_axes.flatten()
        edge_map  =[(node_map(i),j) for j in range(4) for i in range(j+2)]
        for i,((in_node,current_node),ax, data) in enumerate(zip(edge_map,graph_axes,reduce_weight)):
            ax.set_ylim([0.0001,1.5])
            ax.set_yticks([])
            ax.set_xticks([])
            ax.bar(range(len(data)),data)
            ...
            if i == 13:
                ax.set_xticks(range(len(data)))
                ax.set_xticklabels(self.op_names, fontsize=28)
        return graph_fig_normal,graph_fig_reduce
