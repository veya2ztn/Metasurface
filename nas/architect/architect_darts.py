...
class ArchitectDARTS(Architect):
    def __init__(self, model, args, writer):
        ...
        self.history = History(model, self, to_save=("alphas", "l2_norm", "l2_norm_from_init"))
        self.history.dict["grads"] = {}
        for v in ["alphas", "edges"]:
            self.history.dict["grads"][v] = {}
            for ct in self.cell_types:
                self.history.dict["grads"][v][ct] = []
        self.alpha_shape_reduce    = model.alpha_shape_reduce
        self.betas_shape_reduce    = model.betas_shape_reduce
        self.alpha_shape_normal    = model.alpha_shape_normal
        self.betas_shape_normal    = model.betas_shape_normal
    ...
    def initialize_alphas(self):
        ...
        for ct in self.cell_types:
            if ct =="normal":
                self.alphas[ct] = Variable(1e-3 * torch.randn(*self.alpha_shape_normal).cuda(), requires_grad=True)
            elif ct =="reduce":
                self.alphas[ct] = Variable(1e-3 * torch.randn(*self.alpha_shape_reduce).cuda(), requires_grad=True)
        self._arch_parameters = [self.alphas[ct] for ct in self.cell_types]

    def initialize_edge_weights(self):
        for ct in self.cell_types:
            if ct =="normal":
                self.edges[ct] = Variable(torch.ones(*self.betas_shape_normal).cuda(), requires_grad=False)
            elif ct =="reduce":
                self.edges[ct] = Variable(torch.ones(*self.betas_shape_reduce).cuda(), requires_grad=False)
