...
def normalize(x, dim, min_v=1e-5,p_min=np.exp(-2),p_max=1):
    x = torch.clamp(x, min=min_v)
    x = x / x.sum(dim=dim, keepdim=True)
    #below method is our custom attempt, and wont improve performance.
    #x = (p_max-p_min)*x + p_min
    #x = x / x.sum(dim=dim, keepdim=True)
    return x
...
class ArchitectEDARTS(Architect):
    ...
    def initialize_alphas(self):
        ...
        for ct in self.cell_types:
            self.alphas[ct] = Variable(
                normalize(torch.rand(k, num_ops).cuda(), dim=-1), requires_grad=True
            )
            ...
            for ct in self.cell_types:
                ...
                if var_name == "alphas":
                    ...
                    for i in range(self.n_nodes):
                        ...
                        offset += n_inputs
                        n_inputs += 1
                    p.data = p.data / (node_weights + 1e-5)
    ...
    def _backward_step(self, input_valid, target_valid):
        #entropic:measure the distance away from the uniform distribution.
        entropic_reg = 0
        for ct in self.cell_types:
            #alphas will be normalized every stop so alphas is the probilitiy
            p = self.alphas[ct]
            k,n = p.shape
            entropic_reg += (-torch.sum( p* torch.log(p) )-0.5*np.log(n*(n-1)))**2/k
            #let the probilitiy fall into the largest probilitiy density surface.
        entropic_reg = entropic_reg/len(self.cell_types)
        curve_loss   = self.model._loss(input_valid, target_valid)
        self.writer.add_scalars('architect_loss',{'loss': curve_loss,'entrop': entropic_reg},self.steps)
        #loss = loss - self.weight_decay * entropic_reg
        #self.annealing_coef  = 0.3*self.weight_decay + 0.7*self.weight_decay*np.exp(-self.steps/30)
        #dynamic_weight_decay = self.annealing_coef * 1
        #loss = curve_loss + dynamic_weight_decay * entropic_reg
        loss = curve_loss + self.weight_decay * entropic_reg
        loss.backward()
        MSE_estimated_max = 16 # half of the #slot = 32 --> 32/2 = 16
        self.last_rate  = (curve_loss.item() / MSE_estimated_max)
